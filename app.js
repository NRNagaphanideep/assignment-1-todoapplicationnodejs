...
const hasPriorityAndStatusProperties = (requestQuery) => {
 return (
 requestQuery.priority !== undefined && requestQuery.status !==
undefined
 );
};
...
const hasCategoryAndStatus = (requestQuery) => {
 return (
 requestQuery.category !== undefined && requestQuery.status !==
undefined
 );
};
const hasCategoryAndPriority = (requestQuery) => {
 return (
 requestQuery.category !== undefined && requestQuery.priority
!== undefined
 );
};
...
app.get("/todos/", async (request, response) => {
 ...
 if (priority === "HIGH" || priority === "MEDIUM" || priority ===
"LOW") {
 if (
 status === "TO DO" ||
 status === "IN PROGRESS" ||
...
outPutResult(eachItem)));
 ...
 if (status === "TO DO" || status === "IN PROGRESS" || status ===
"DONE") {
...
due_date='${newDate}';`;
 ...
 response.send(responseResult.map((eachItem) =>
outPutResult(eachItem)));
 } else {
 response.status(400);
...
app.post("/todos/", async (request, response) => {
 ...
 if (priority === "HIGH" || priority === "LOW" || priority ===
"MEDIUM") {
 if (status === "TO DO" || status === "IN PROGRESS" || status ===
"DONE") {
 if (
 ...
 const postNewDueDate = format(new Date(dueDate), "yyyyMM-dd");
 const postTodoQuery = `
 INSERT INTO
...
status='${status}', category='${category}',
 ...
 if (isMatch(dueDate, "yyyy-MM-dd")) {
 const newDueDate = format(new Date(dueDate), "yyyy-MMdd");
 updateTodoQuery = `
 UPDATE todo SET todo='${todo}', priority='${priority}',
status='${status}', category='${category}',
